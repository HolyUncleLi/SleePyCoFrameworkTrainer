...
class TemporalContextModule(nn.Module):
    def __init__(self, config, feature_dim, num_filter_scales):
        ...
        seq_length = config['length_time_series'] // (2 ** (num_filter_scales - 1)) - (config['kernel_scales'] - 1)
        seq_length = 746
...
class MSA_CNN(nn.Module):
    def __init__(self, config):
        ...
        if config.get('num_attention_layers', 0) > 0:
            self.tcm = TemporalContextModule(self.config, out_dim, self.msm.num_filter_scales)
